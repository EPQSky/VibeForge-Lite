...
本仓库刻意分开“可分发工作流”和“目标项目配置”：

- Plugin 边界：`.codex-plugin/plugin.json` 与 `skills/`，由 Codex 安装和发现。
- Project 边界：`templates/project/` 与 `$vibe-init`，只向目标仓库写入持久规则、文档约定和状态，不重复复制 Plugin skills。

这样可以独立升级工作流，不让每个业务仓库维护一份 prompt 副本。确需固定或覆盖 skill 时，目标项目才使用官方仓库级 `.agents/skills/`。
